...
import { portfolioFooterData } from "@/features/portfolio/portfolio.data";
    ...
    <footer className="relative overflow-hidden border-t border-white/10 bg-slate-950 px-5 pb-7 pt-10 text-white sm:px-6 lg:px-8">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-xl flex-col items-center text-center">
        ...
        <h2 className="mt-6 text-2xl font-black leading-tight tracking-[-0.05em] text-white sm:text-4xl">
          {data.title || "Punya project yang ingin ditampilkan?"}
        </h2>

        <p className="mt-3 max-w-md text-sm font-medium leading-7 text-slate-400 sm:text-base">
          {data.description ||
            "Portfolio Nexarin akan terus dikembangkan untuk menampilkan project digital dan karya by-rins."}
        </p>
        ...
        <Link
          ...
          className="mt-6 inline-flex min-h-14 items-center justify-center gap-3 rounded-[30px] border border-emerald-400/20 bg-emerald-400/20 py-2 pl-2 pr-7 text-sm font-black text-white shadow-xl shadow-emerald-400/10 transition hover:bg-emerald-400 hover:text-slate-950"
        >
          <SupportIcon />
          <span>{cta.label || "Hubungi Nexarin"}</span>
        </Link>
