...
import { contactFooterData } from "@/features/contact/contact.data";
    ...
    <footer className="relative overflow-hidden border-t border-white/10 bg-slate-950 px-5 pb-7 pt-10 text-white sm:px-6 lg:px-8">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-xl flex-col items-center text-center">
        ...
        <h2 className="mt-6 text-2xl font-black leading-tight tracking-[-0.05em] text-white sm:text-4xl">
          {data.title || "Siap ngobrolin kebutuhan digital?"}
        </h2>

        <p className="mt-3 max-w-md text-sm font-medium leading-7 text-slate-400 sm:text-base">
          {data.description ||
            "Kirim kebutuhan awalnya dulu. Nanti contact bisa dihubungkan ke WhatsApp, email, database, atau admin dashboard."}
        </p>

        <div className="mt-6 grid w-full gap-3 sm:grid-cols-2">
          <Link
            href={primary.href || "#contact-form"}
          ...
          >
            <SupportIcon />
            <span>{primary.label || "Kirim Pesan"}</span>
          </Link>

          <Link
            href={secondary.href || "/products"}
            className="inline-flex min-h-14 items-center justify-center rounded-[30px] border border-white/10 bg-white/[0.045] px-5 py-3 text-sm font-black text-white shadow-xl shadow-black/10 transition hover:border-emerald-400/25 hover:bg-emerald-400/10"
          >
            {secondary.label || "Lihat Products"}
          </Link>
        </div>
