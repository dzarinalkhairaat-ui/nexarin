...
import { UploadCloud, FileType, CheckCircle2, ArrowLeft, Loader2, Download, FilePlus, X, Presentation , Shield, Zap, Sparkles } from "lucide-react";
    ...
    <main className="min-h-screen bg-slate-950 pt-10 sm:pt-16 text-slate-300 selection:bg-orange-500/30 relative">
      <div className="max-w-5xl mx-auto px-6 pb-20 relative z-10">
        <div className="mb-10 text-center animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
          <div className="inline-flex items-center justify-center p-4 rounded-3xl bg-orange-500/10 text-orange-500 mb-4 shadow-[0_0_30px_rgba(249,115,22,0.15)] ring-1 ring-orange-500/20">
            <Presentation className="w-10 h-10" />
          </div>
          
          <div className="flex items-center justify-center gap-4 mb-4">
            <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white">
              Konversi <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-400">PowerPoint ke PDF</span>
            </h1>
          </div>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Buat tayangan slide presentasi Anda lebih mudah dilihat dan dibagikan dengan mengubahnya menjadi dokumen PDF yang rapi.{' '}
            <button 
              onClick={() => setShowInfoModal(true)}
              className="text-orange-500 hover:text-orange-400 font-semibold underline decoration-orange-500/30 hover:decoration-orange-400 underline-offset-4 transition-colors"
            >
          ...
          {selectedFiles.length > 0 ? (
              ...
              <div className="flex flex-wrap justify-center gap-3 mb-6">
                {selectedFiles.map((file, idx) => (
                  <div key={idx} className="relative w-16 h-20 bg-white/5 border border-white/10 rounded-xl flex flex-col items-center justify-center shadow-xl shadow-black/20 group-hover:scale-105 transition-transform">
                    ...
                    <FileType className="w-6 h-6 text-orange-500 opacity-80 mb-1" strokeWidth={1.5} />
                ...
                {isMulti && processingState === 'idle' && (
                  <label className="cursor-pointer relative w-16 h-20 bg-white/[0.02] hover:bg-white/[0.05] border border-dashed border-white/20 rounded-xl flex items-center justify-center transition-colors">
                    ...
                    <input type="file" multiple className="hidden" accept=".ppt,.pptx,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation" onChange={(e) => {
              ...
              <div className="flex flex-col-reverse sm:flex-row items-center justify-center gap-4 w-full max-w-md mx-auto mt-2">
                {processingState === 'idle' || processingState === 'error' ? (
                  ...
                  <div className="flex flex-col gap-6 items-center w-full mt-4">
                    <p className="text-orange-500 font-bold text-xl">File berhasil diproses!</p>
            ...
            >
              ...
              <p className="text-slate-400 text-center text-sm mb-8 max-w-sm px-4 leading-relaxed">
                Anda bisa memilih file presentasi PowerPoint (PPT/PPTX) yang ingin diubah menjadi PDF secara otomatis.
              </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[2.5rem] w-full max-w-lg p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-4 mb-6 pr-12">
              <div className="p-3 bg-orange-500/10 rounded-2xl text-orange-500 shrink-0">
                <Presentation className="w-6 h-6 sm:w-8 sm:h-8" />
              </div>
              <h2 className="text-xl sm:text-2xl font-black text-white leading-tight">Fungsi Fitur PowerPoint ke PDF</h2>
            </div>
            
            <div className="space-y-4 text-slate-300 leading-relaxed text-sm sm:text-base">
              <p>
                Alat <strong>PowerPoint to PDF</strong> didesain untuk merubah tayangan slide presentasi Anda (PPT/PPTX) menjadi format dokumen portabel (PDF) yang jauh lebih ringan dan aman dibagikan.
              </p>
              
              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-orange-500 shrink-0" /> Tampilan Terkunci Sempurna
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Semua elemen presentasi seperti font, tata letak teks, gambar, dan bentuk geometris akan dipertahankan dengan akurasi 100%, sehingga presentasi Anda tidak akan berantakan saat dibuka klien.
                </p>
              </div>

              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-orange-500 shrink-0" /> Ringan & Mudah Diakses
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  File PDF memiliki ukuran yang lebih padat, sehingga sangat mudah untuk di-upload atau dikirim melalui Email maupun WhatsApp dengan cepat.
                </p>
              </div>
            </div>
            ...
            <div className="mt-10 flex flex-col items-center">
              <button 
                onClick={() => setShowInfoModal(false)}
                className="px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl transition-colors w-full"
              >
                Saya Mengerti
              </button>
            </div>
