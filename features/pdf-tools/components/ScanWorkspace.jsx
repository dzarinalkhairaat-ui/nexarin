...
import { UploadCloud, FileType, CheckCircle2, ArrowLeft, Loader2, Download, FilePlus, X, Scan , Shield, Zap, Sparkles } from "lucide-react";
...
export default function ScanWorkspace() {
  ...
  return (
    <main className="min-h-screen bg-slate-950 pt-10 sm:pt-16 text-slate-300 selection:bg-red-500/30 relative">
      <div className="max-w-5xl mx-auto px-6 pb-20 relative z-10">
        <div className="mb-10 text-center animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
          <div className="inline-flex items-center justify-center p-4 rounded-3xl bg-red-500/10 text-red-400 mb-6 shadow-[0_0_30px_rgba(255,255,255,0.1)] ring-1 ring-red-500/20">
            <Scan className="w-10 h-10" />
          </div>
          
          <div className="flex items-center justify-center gap-4 mb-4">
            <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white">
              Scan ke <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-rose-400">PDF</span>
            </h1>
          </div>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Ubah foto dokumen fisik Anda menjadi file PDF berkualitas tinggi yang siap dikirim dan dicetak.{' '}
            <button 
          ...
          {selectedFiles.length > 0 ? (
              ...
              <div className="flex flex-wrap justify-center gap-3 mb-6">
                {selectedFiles.map((file, idx) => (
                  <div key={idx} className="relative w-16 h-20 bg-white/5 border border-white/10 rounded-xl flex flex-col items-center justify-center shadow-xl shadow-black/20 group-hover:scale-105 transition-transform">
                    {idx === 0 && (
                      <div className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-red-500 flex items-center justify-center text-slate-950 shadow-lg shadow-red-500/30">
                        <CheckCircle2 className="w-4 h-4" />
                      </div>
                    )}
                    <FileType className="w-6 h-6 text-red-400 opacity-80 mb-1" strokeWidth={1.5} />
            ...
            >
              ...
              <p className="text-slate-400 text-center text-sm mb-8 max-w-sm px-4 leading-relaxed">
                Anda bisa memilih file pindaian gambar (JPG/PNG) yang ingin diubah menjadi PDF secara otomatis.
              </p>
              
              <button 
                onClick={() => document.getElementById('file-upload')?.click()}
                className="px-8 py-4 bg-red-500 text-white font-bold rounded-2xl hover:bg-red-600 transition-all active:scale-95 shadow-lg shadow-red-500/30 hover:shadow-xl hover:shadow-red-500/50 flex items-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[2.5rem] w-full max-w-lg p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-4 mb-6 pr-12">
              ...
              <h2 className="text-xl sm:text-2xl font-black text-white leading-tight">Fungsi Fitur Scan ke PDF</h2>
            </div>
            
            <div className="space-y-4 text-slate-300 leading-relaxed text-sm sm:text-base">
              <p>
                Alat <strong>Scan ke PDF</strong> adalah solusi cepat untuk mendigitalkan dokumen fisik seperti struk, tagihan, ijazah, atau kontrak langsung menjadi dokumen PDF yang rapi.
              </p>
              
              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-red-400 shrink-0" /> Pembersihan Otomatis
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Secara cerdas merapikan perspektif, kontras, dan tingkat kecerahan foto agar tampak seolah-olah dipindai oleh mesin *scanner* sungguhan.
                </p>
              </div>

              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-red-400 shrink-0" /> Praktis & Cepat
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Tidak perlu repot mencari mesin fotokopi. Cukup foto menggunakan *smartphone*, unggah gambar kesini, lalu unduh hasilnya.
                </p>
              </div>
            </div>
            ...
            <div className="mt-10 flex flex-col items-center">
              <button 
                onClick={() => setShowInfoModal(false)}
                className="px-6 py-3 bg-red-500 hover:bg-red-600 text-slate-950 font-bold rounded-xl transition-colors w-full"
              >
                Saya Mengerti
              </button>
            </div>
