...
import { UploadCloud, FileType, CheckCircle2, ArrowLeft, Loader2, Download, FilePlus, X, Table , Shield, Zap, Sparkles } from "lucide-react";
    ...
    <main className="min-h-screen bg-slate-950 pt-10 sm:pt-16 text-slate-300 selection:bg-emerald-500/30 relative">
      <div className="max-w-5xl mx-auto px-6 pb-20 relative z-10">
        <div className="mb-10 text-center animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
          <div className="inline-flex items-center justify-center p-4 rounded-3xl bg-emerald-500/10 text-emerald-400 mb-6 shadow-[0_0_30px_rgba(255,255,255,0.1)] ring-1 ring-emerald-500/20">
            <Table className="w-10 h-10" />
          </div>
          
          <div className="flex items-center justify-center gap-4 mb-4">
            <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white">
              Konversi <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">Excel ke PDF</span>
            </h1>
          </div>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Buat tabel spreadsheet Anda lebih mudah dibaca, dicetak, dan dibagikan dengan mengubahnya menjadi dokumen PDF.{' '}
            <button 
          ...
          {selectedFiles.length > 0 ? (
              ...
              <div className="flex flex-wrap justify-center gap-3 mb-6">
                ...
                {isMulti && processingState === 'idle' && (
                  <label className="cursor-pointer relative w-16 h-20 bg-white/[0.02] hover:bg-white/[0.05] border border-dashed border-white/20 rounded-xl flex items-center justify-center transition-colors">
                    ...
                    <input type="file" multiple className="hidden" accept=".xls,.xlsx,.csv,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,text/csv" onChange={(e) => {
            ...
            >
              ...
              <p className="text-slate-400 text-center text-sm mb-8 max-w-sm px-4 leading-relaxed">
                Anda bisa memilih file spreadsheet Excel (XLS/XLSX/CSV) yang ingin diubah menjadi PDF secara otomatis.
              </p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[2.5rem] w-full max-w-lg p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-4 mb-6 pr-12">
              ...
              <h2 className="text-xl sm:text-2xl font-black text-white leading-tight">Fungsi Fitur Excel ke PDF</h2>
            </div>
            
            <div className="space-y-4 text-slate-300 leading-relaxed text-sm sm:text-base">
              <p>
                Alat <strong>Excel to PDF</strong> dikhususkan untuk merubah file spreadsheet Anda (XLS/XLSX) menjadi format PDF agar data tabel tidak bergeser saat dicetak atau dibuka oleh orang lain.
              </p>
              
              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" /> Pertahankan Format Tabel
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Lebar kolom, garis batas, warna *cell*, hingga grafik yang Anda buat di Excel akan dikonversi dengan persis seperti tampilan aslinya, sehingga data mudah dibaca.
                </p>
              </div>

              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" /> Orientasi yang Cerdas
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Secara cerdas sistem akan menyusun tabel ke dalam ukuran halaman PDF standar sehingga seluruh baris dan kolom masuk ke dalam kertas tanpa terpotong berantakan.
                </p>
              </div>
            </div>
            ...
            <div className="mt-10 flex flex-col items-center">
              <button 
                onClick={() => setShowInfoModal(false)}
                className="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl transition-colors w-full"
              >
                Saya Mengerti
              </button>
            </div>
