...
import { UploadCloud, FileType, CheckCircle2, ArrowLeft, Loader2, Download, FilePlus, X, Archive , Shield, Zap, Sparkles } from "lucide-react";
...
export default function PdfToPdfaWorkspace() {
  const slug = "pdf-to-pdfa";
  ...
  return (
    <main className="min-h-screen bg-slate-950 pt-10 sm:pt-16 text-slate-300 selection:bg-slate-500/30 relative">
      <div className="max-w-5xl mx-auto px-6 pb-20 relative z-10">
        <div className="mb-10 text-center animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
          <div className="inline-flex items-center justify-center p-4 rounded-3xl bg-slate-500/10 text-slate-400 mb-6 shadow-[0_0_30px_rgba(255,255,255,0.1)] ring-1 ring-slate-500/20">
            <Archive className="w-10 h-10" />
          </div>
          
          <div className="flex items-center justify-center gap-4 mb-4">
            <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white">
              PDF ke <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-400 to-gray-400">PDF/A</span>
            </h1>
          </div>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Konversi dokumen PDF Anda menjadi format standar PDF/A untuk pengarsipan digital jangka panjang.{' '}
            <button 
              onClick={() => setShowInfoModal(true)}
              className="text-slate-400 hover:text-slate-300 font-semibold underline decoration-slate-500/30 hover:decoration-slate-400 underline-offset-4 transition-colors"
            >
        ...
        <div className="bg-slate-900/50 border border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl backdrop-blur-xl relative overflow-hidden">
          ...
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-3/4 h-64 bg-slate-500/10 blur-[100px] pointer-events-none rounded-full" />
          ...
          {selectedFiles.length > 0 ? (
              ...
              <div className="flex flex-wrap justify-center gap-3 mb-6">
                {selectedFiles.map((file, idx) => (
                  <div key={idx} className="relative w-16 h-20 bg-white/5 border border-white/10 rounded-xl flex flex-col items-center justify-center shadow-xl shadow-black/20 group-hover:scale-105 transition-transform">
                    {idx === 0 && (
                      <div className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-slate-500 flex items-center justify-center text-slate-950 shadow-lg shadow-slate-500/30">
                        <CheckCircle2 className="w-4 h-4" />
                      </div>
                    )}
                    <FileType className="w-6 h-6 text-slate-400 opacity-80 mb-1" strokeWidth={1.5} />
                ...
                {processingState === 'idle' || processingState === 'error' ? (
                  ...
                  <div className="w-full max-w-md mx-auto">
                    ...
                    <div className="h-3 w-full bg-slate-800 rounded-full overflow-hidden shadow-inner border border-white/5">
                      <div 
                        className="h-full bg-slate-500 rounded-full transition-all duration-300 relative"
                  ...
                  <div className="flex flex-col gap-6 items-center w-full mt-4">
                    <p className="text-slate-400 font-bold text-xl">File berhasil diproses!</p>
                    
                    <button 
                      onClick={handleDownload} 
                      className="w-full sm:w-auto px-10 py-4 bg-gradient-to-r from-slate-500 to-slate-600 hover:from-slate-600 hover:to-slate-700 text-white font-bold rounded-full transition-all duration-300 shadow-[0_10px_20px_-10px_rgba(255,255,255,0.2)] hover:shadow-[0_10px_30px_-10px_rgba(255,255,255,0.4)] hover:-translate-y-0.5 active:scale-95 flex items-center justify-center gap-2 text-lg border border-slate-400/20"
                    >
            ...
            <div 
              className={`relative flex flex-col items-center justify-center border-2 border-dashed rounded-[2rem] p-12 transition-all duration-300 ${
                isDragging 
                  ? 'border-slate-400 bg-slate-500/10 scale-[1.02]' 
            ...
            >
              ...
              <div className="flex h-24 w-24 items-center justify-center rounded-3xl bg-slate-800/80 border border-slate-700 shadow-xl mb-6 text-slate-400 group-hover:scale-110 transition-transform">
              ...
              <button 
                ...
                className="px-8 py-4 bg-slate-500 text-white font-bold rounded-2xl hover:bg-slate-600 transition-all active:scale-95 shadow-lg shadow-slate-500/30 hover:shadow-xl hover:shadow-slate-500/50 flex items-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[2.5rem] w-full max-w-lg p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-4 mb-6 pr-12">
              <div className="p-3 bg-slate-500/10 rounded-2xl text-slate-400 shrink-0">
                <Archive className="w-6 h-6 sm:w-8 sm:h-8" />
              </div>
              <h2 className="text-xl sm:text-2xl font-black text-white leading-tight">Fungsi Fitur PDF ke PDF/A</h2>
            </div>
            
            <div className="space-y-4 text-slate-300 leading-relaxed text-sm sm:text-base">
              <p>
                Alat <strong>PDF ke PDF/A</strong> dirancang khusus untuk memastikan dokumen Anda akan selalu terlihat sama persis, puluhan bahkan ratusan tahun dari sekarang.
              </p>
              
              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-slate-400 shrink-0" /> Standar Pengarsipan Global (ISO)
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  PDF/A adalah format standar ISO yang diwajibkan oleh banyak instansi hukum, pemerintah, dan perpustakaan di seluruh dunia untuk tujuan pengarsipan (*archiving*).
                </p>
              </div>

              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-slate-400 shrink-0" /> Dokumen Berdiri Sendiri
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Seluruh *font*, warna, dan elemen visual akan di-embed langsung secara absolut ke dalam file, sehingga tidak terpengaruh oleh ketiadaan aplikasi atau *font* di komputer masa depan.
                </p>
              </div>
            </div>
            ...
            <div className="mt-10 flex flex-col items-center">
              <button 
                onClick={() => setShowInfoModal(false)}
                className="px-6 py-3 bg-slate-500 hover:bg-slate-600 text-slate-950 font-bold rounded-xl transition-colors w-full"
              >
                Saya Mengerti
              </button>
            </div>
