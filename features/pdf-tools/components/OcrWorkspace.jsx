...
import { UploadCloud, FileType, CheckCircle2, ArrowLeft, Loader2, Download, FilePlus, X, ScanText , Shield, Zap, Sparkles } from "lucide-react";
        ...
        <div className="mb-10 text-center animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
          ...
          <div className="flex items-center justify-center gap-4 mb-4">
            <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white">
              OCR <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-amber-400">PDF</span>
            </h1>
          </div>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Ubah dokumen hasil scan menjadi teks yang dapat diedit dan dicari menggunakan teknologi pengenalan karakter optik (OCR).{' '}
            <button 
              ...
              <div className="flex flex-col-reverse sm:flex-row items-center justify-center gap-4 w-full max-w-md mx-auto mt-2">
                {processingState === 'idle' || processingState === 'error' ? (
                  <>
                    ...
                    <button onClick={processFile} className="w-full sm:flex-1 px-10 py-4 bg-gradient-to-r from-yellow-500 via-yellow-600 to-yellow-600 hover:from-yellow-600 hover:via-yellow-600 hover:to-yellow-700 text-white font-bold rounded-2xl transition-all duration-300 shadow-[0_10px_30px_-10px_rgba(255,255,255,0.15)] hover:shadow-[0_10px_40px_-10px_rgba(255,255,255,0.3)] hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-3 text-lg border border-yellow-400/30">
                      Proses File
                    </button>
                  </>
          ...
          <div className="relative bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[2.5rem] w-full max-w-lg p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-4 mb-6 pr-12">
              <div className="p-3 bg-yellow-500/10 rounded-2xl text-yellow-400 shrink-0">
                <ScanText className="w-6 h-6 sm:w-8 sm:h-8" />
              </div>
              <h2 className="text-xl sm:text-2xl font-black text-white leading-tight">Fungsi Fitur OCR PDF</h2>
            </div>
            
            <div className="space-y-4 text-slate-300 leading-relaxed text-sm sm:text-base">
              <p>
                Alat <strong>OCR (Optical Character Recognition)</strong> digunakan untuk mengekstrak teks yang terjebak di dalam gambar hasil *scan*, sehingga bisa diblok, *copy*, atau dicari.
              </p>
              
              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-yellow-400 shrink-0" /> Ekstraksi Teks Pintar
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Mengubah dokumen PDF yang hanya berupa gambar (seperti hasil *scan* atau foto buku) menjadi dokumen *searchable* PDF yang teksnya bisa Anda *copy-paste* ke Word atau aplikasi lain.
                </p>
              </div>

              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-yellow-400 shrink-0" /> Sangat Akurat
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Mesin OCR kami mengenali struktur paragraf, tabel, dan spasi dengan akurasi tinggi, serta mendukung pengenalan karakter dalam berbagai bahasa.
                </p>
              </div>
            </div>
