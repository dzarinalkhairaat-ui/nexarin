...
import { UploadCloud, FileType, CheckCircle2, ArrowLeft, Loader2, Download, FilePlus, X, PenTool , Shield, Zap, Sparkles } from "lucide-react";
    ...
    <main className="min-h-screen bg-slate-950 pt-10 sm:pt-16 text-slate-300 selection:bg-indigo-500/30 relative">
      <div className="max-w-5xl mx-auto px-6 pb-20 relative z-10">
        <div className="mb-10 text-center animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
          <div className="inline-flex items-center justify-center p-4 rounded-3xl bg-indigo-500/10 text-indigo-400 mb-6 shadow-[0_0_30px_rgba(255,255,255,0.1)] ring-1 ring-indigo-500/20">
            <PenTool className="w-10 h-10" />
          </div>
          
          <div className="flex items-center justify-center gap-4 mb-4">
            <h1 className="text-4xl md:text-5xl font-black tracking-tight text-white">
              Tanda Tangan <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-violet-400">PDF</span>
            </h1>
          </div>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Bubuhi tanda tangan elektronik (e-Sign) pada dokumen PDF Anda secara aman tanpa harus mencetaknya.{' '}
            <button 
        ...
        <div className="bg-slate-900/50 border border-white/10 rounded-[2.5rem] p-8 md:p-12 shadow-2xl backdrop-blur-xl relative overflow-hidden">
          ...
          {selectedFiles.length > 0 ? (
              ...
              <div className="flex flex-wrap justify-center gap-3 mb-6">
                {selectedFiles.map((file, idx) => (
                  <div key={idx} className="relative w-16 h-20 bg-white/5 border border-white/10 rounded-xl flex flex-col items-center justify-center shadow-xl shadow-black/20 group-hover:scale-105 transition-transform">
                    {idx === 0 && (
                      <div className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-indigo-500 flex items-center justify-center text-slate-950 shadow-lg shadow-indigo-500/30">
                        <CheckCircle2 className="w-4 h-4" />
                      </div>
                    )}
                    <FileType className="w-6 h-6 text-indigo-400 opacity-80 mb-1" strokeWidth={1.5} />
              ...
              <div className="flex justify-center gap-4">
                {processingState === 'idle' || processingState === 'error' ? (
                  ...
                  <div className="flex flex-col gap-6 items-center w-full mt-4">
                    <p className="text-indigo-400 font-bold text-xl">File berhasil diproses!</p>
            ...
            >
              ...
              <div className="flex h-24 w-24 items-center justify-center rounded-3xl bg-slate-800/80 border border-slate-700 shadow-xl mb-6 text-indigo-400 group-hover:scale-110 transition-transform">
              ...
              <button 
                ...
                className="px-8 py-4 bg-indigo-500 text-white font-bold rounded-2xl hover:bg-indigo-600 transition-all active:scale-95 shadow-lg shadow-indigo-500/30 hover:shadow-xl hover:shadow-indigo-500/50 flex items-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[2.5rem] w-full max-w-lg p-8 sm:p-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.7)] animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center gap-4 mb-6 pr-12">
              ...
              <h2 className="text-xl sm:text-2xl font-black text-white leading-tight">Fungsi Fitur Tanda Tangan PDF</h2>
            </div>
            
            <div className="space-y-4 text-slate-300 leading-relaxed text-sm sm:text-base">
              <p>
                Alat <strong>Sign PDF</strong> memungkinkan Anda membubuhkan tanda tangan elektronik ke dalam dokumen tanpa perlu mencetaknya terlebih dahulu.
              </p>
              
              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 shrink-0" /> Hemat Waktu & Kertas
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Tidak perlu lagi mencetak (print), menandatangani, lalu memindai (scan) ulang dokumen. Lakukan semuanya murni secara digital dalam satu tempat.
                </p>
              </div>

              <div className="bg-slate-950 rounded-xl p-4 sm:p-5 border border-slate-800">
                <h4 className="text-white font-bold mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-indigo-400 shrink-0" /> Keamanan & Validitas
                </h4>
                <p className="text-xs sm:text-sm text-slate-400">
                  Bentuk tanda tangan elektronik semakin diakui secara legal untuk berbagai kebutuhan surat menyurat, laporan kerja, maupun perjanjian formal.
                </p>
              </div>
            </div>
