...
export default function AdminLoginPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#080D1A] flex flex-col items-center justify-center p-4 sm:p-6 text-slate-100">
      {/* Central Admin Login Card */}
      <div className="w-full max-w-md bg-[#0B1120] rounded-3xl border border-[#1E293B] p-8 sm:p-10 shadow-2xl space-y-6">
        {/* Brand & Identity Header */}
        <div className="text-center space-y-3">
          <div className="w-14 h-14 rounded-2xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center mx-auto text-cyan-400 shadow-sm">
            <ShieldCheck className="w-7 h-7" />
          </div>

          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-900 border border-slate-800 text-[10px] font-mono font-bold text-cyan-400 uppercase tracking-wider">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <input
                ...
                className="w-full rounded-xl border border-[#1E293B] bg-[#080D1A] text-slate-100 px-4 py-2.5 text-sm transition-all focus:outline-none focus:ring-2 focus:ring-[#2DD4F5] focus:border-transparent placeholder:text-slate-600"
              />
            </div>
          </div>
          ...
          <Button
            ...
            variant="primary"
            size="md"
            className="w-full font-extrabold text-sm text-slate-950 shadow-md shadow-cyan-500/10 mt-2"
            disabled={loading}
          >
            {loading ? (
