      ...
      {modules.map((mod, modIdx) => {
        ...
        return (
          ...
          >
            ...
            >
              <div className="space-y-1">
                ...
                {mod.description && !isClassroomView && (
                  <p className="text-xs text-[#94A3B8] line-clamp-1">{mod.description}</p>
                )}
              </div>
            ...
            {/* Lesson items */}
            {isOpen && (
              <div className="border-t border-white/[0.08] divide-y divide-white/[0.06] bg-[#0B1120]/60">
                  ...
                  return (
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive
                          ? "bg-[#2DD4F5]/10 text-[#2DD4F5] font-bold border-l-2 border-[#2DD4F5]"
                    ...
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0 pr-2">
                        {isDone ? (
                          ...
                          <PlayCircle className={cn("w-4 h-4 flex-shrink-0", isActive ? "text-[#2DD4F5]" : "text-[#64748B] group-hover:text-[#F8FAFC]")} />
                        )}
                        <span className="text-[11px] font-mono text-[#64748B]">
                          {lesIdx + 1 < 10 ? `0${lesIdx + 1}` : lesIdx + 1}.
                        </span>
                        <span className={cn("truncate", isActive && "text-[#F8FAFC]")}>
                          {lesson.title}
                        </span>
                      </div>

                      <div className="flex items-center gap-2 flex-shrink-0 font-mono text-[11px] text-[#64748B]">
                        {lesson.isPreviewAvailable && (
                          <span className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase bg-[#7CF2C3]/10 text-[#7CF2C3] border border-[#7CF2C3]/20">
                            Preview
                          </span>
                        )}
