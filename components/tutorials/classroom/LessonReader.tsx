...
export function LessonReader({
  ...
  return (
    <div className="space-y-8">
      ...
      {/* MAIN LESSON PAPER CANVAS */}
      <article
        className="rounded-3xl p-6 sm:p-10 backdrop-blur-xl border border-transparent space-y-8 shadow-2xl overflow-hidden"
        style={{
          background:
            "linear-gradient(180deg, rgba(15, 23, 42, 0.92), rgba(11, 17, 32, 0.88)) padding-box, linear-gradient(120deg, rgba(255, 255, 255, 0.15), rgba(45, 212, 245, 0.25), rgba(124, 242, 195, 0.10)) border-box",
      ...
      >
        ...
        <div className="space-y-4 pb-6 border-b border-white/[0.08]">
          <div className="flex flex-wrap items-center gap-2.5 text-xs font-mono">
            ...
            <span className="flex items-center gap-1 text-slate-400 bg-white/[0.04] px-2.5 py-1 rounded-full border border-white/[0.08]">
              ...
              {currentLesson.duration || "20 min"} Durasi Belajar
            </span>
          </div>
          ...
          <div className="flex items-center gap-3 pt-1 text-xs text-slate-400 font-mono">
            ...
            />
            <span>Instruktur: <strong className="text-white">{course.instructor.name}</strong> ({course.instructor.role})</span>
          </div>
        </div>
        ...
        {currentLesson.exercises && currentLesson.exercises.length > 0 && (
          <div className="p-6 rounded-2xl bg-cyan-500/10 border border-cyan-500/25 space-y-4">
              ...
              {currentLesson.exercises.map((ex, idx) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full text-left p-3 rounded-xl text-xs sm:text-sm flex items-start gap-3 transition-all",
                      isChecked
                  ...
                  >
                    {isChecked ? (
                      <CheckSquare className="w-4 h-4 text-[#7CF2C3] shrink-0 mt-0.5" />
                    ) : (
                      <Square className="w-4 h-4 text-slate-500 shrink-0 mt-0.5" />
                    )}
                    <span className={cn("flex-1", isChecked && "line-through opacity-80")}>{ex}</span>
                  </button>
                );
              })}
            </div>
        ...
        <div className="pt-6 border-t border-white/[0.08] flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          {nextLesson ? (
            <Link
              ...
              className="w-full sm:w-auto flex items-center justify-between sm:justify-end gap-2.5 px-5 py-3 rounded-2xl bg-[#2DD4F5] hover:bg-[#2DD4F5]/90 text-slate-950 text-xs font-extrabold shadow-lg shadow-cyan-500/20 transition-all group"
            >
            ...
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl bg-gradient-to-r from-[#2DD4F5] to-[#7CF2C3] text-slate-950 text-xs font-black shadow-lg shadow-emerald-500/20 transition-all"
            >
