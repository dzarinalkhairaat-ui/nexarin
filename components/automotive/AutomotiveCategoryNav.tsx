...
export interface AutomotiveCategoryFilter {
  id: string;
  name: string;
  count?: number;
}

interface AutomotiveCategoryNavProps {
  categories: AutomotiveCategoryFilter[];
  activeCategory: string;
  onSelectCategory: (id: string) => void;
}

export function AutomotiveCategoryNav({
  categories,
  activeCategory,
  onSelectCategory
}: AutomotiveCategoryNavProps) {
  return (
    <nav aria-label="Automotive Subcategories" className="border-b border-white/[0.08] bg-[#0B1120]/95 backdrop-blur-xl sticky top-16 sm:top-20 z-20">
