...
export interface GadgetCategoryFilter {
  id: string;
  name: string;
  count?: number;
}

interface GadgetCategoryNavProps {
  categories: GadgetCategoryFilter[];
  activeCategory: string;
  onSelectCategory: (id: string) => void;
}

export function GadgetCategoryNav({
  categories,
  activeCategory,
  onSelectCategory
}: GadgetCategoryNavProps) {
  return (
    <nav aria-label="Gadget Subcategories" className="border-b border-white/[0.08] bg-[#0B1120]/95 backdrop-blur-xl sticky top-16 sm:top-20 z-20">
